...
function LockScreen({ appLock, onUnlock }) {
  const [pin, setPin] = useState(''),
    ...
    [usePin, setUsePin] = useState(appLock.mode !== 'biometric');
  useEffect(() => {
      ...
      {rows.map((r) => (
        <View key={r.month} style={{ marginBottom: 14 }}>
          ...
          <View style={s.rowTop}>
            ...
            <Text style={s.muted}>{formatINR(r.expense)}</Text>
          </View>
          <View style={s.track}>
          ...
          <View style={[s.rowTop, { marginTop: 6 }]}>
            ...
            <Text style={s.muted}>{formatINR(r.income)}</Text>
          </View>
          <View style={s.track}>
